...
import OutlineArrow from '../../../public/images/outlineArrowDown.svg';
...
const Transactions = () => {
    ...
    return (
        <div className='w-full gap-5 flex-col-start'>
            ...
            <div className="p-3 !pb-6 bg-white shadow-[0px_2px_6px_0px_#1A181E0A] rounded-lg w-full flex-col-start gap-6">
                <div className="w-full gap-3 flex-col-start">
                    ...
                    <div className="w-full flex-between">
                        <div className="w-[calc(100%-135px)] sm:w-[248px] relative">
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" className='absolute -translate-y-1/2 left-4 top-1/2' width="14" height="14" viewBox="0 0 14 14" fill="none">
                                <g clipPath="url(#clip0_91507_83651)">
                        ...
                        <div className="gap-3 flex-center">
                            ...
                            <button className="flex-center gap-1 py-1.5  px-3 h-9 rounded bg-white border cursor-pointer  border-[#D9D9D9] ">
                                <span className="text-[#4D4D4D] pb-1 sm:block hidden  text-base font-normal font-Galano">Sort</span>
                    ...
                    <div className='w-full max-w-full overflow-auto flex-col-start !items-start no-scrollbar'>
                        ...
                        <div className="w-full flex-col-start !items-start">
                            {currentItems.map(({ id, orderAmt, fees, date }, ind) => <TableRow key={ind} id={id} orderAmt={orderAmt} fees={fees} date={date} />)}
                        </div>
                    </div>
                </div>

                {/* pagination */}
                <div className="w-full max-w-full gap-6 overflow-auto no-scrollbar pagination lg:flex-center lg:!justify-center !justify-start flex-start">

                    {/* prev  button */}
                    <button
                        onClick={() => paginate(currentPage - 1)}
                        disabled={currentPage === 1}
                        className="flex-center gap-1.5 py-1.5  px-3 rounded bg-white border cursor-pointer  border-[#D9D9D9] ">
                        <img src={OutlineArrow} alt="" className='rotate-90' />
                        <span className="text-[#4D4D4D] text-sm pb-1 font-normal font-Galano">Previous</span>
                    </button>
                        ...
                        {Array.from({ length: pageCount }, (_, i) => i + 1).map((number) => (
                            <button
                                ...
                                className={` ${number === currentPage
                                    ? "active bg-[#146EB4] border text-white "
                                    : " text-[#4D4D4D]"
                                    } text-sm rounded font-Inter font-normal py-2 px-4 `}
                            >
                                {number}
                            </button>
                        ))}
                    ...
                    <button
                        ...
                        <span className="text-[#4D4D4D] text-sm  pb-1 font-normal font-Galano">Next</span>
                        <img src={OutlineArrow} alt="" className='-rotate-90' />
                    </button>
                </div>
